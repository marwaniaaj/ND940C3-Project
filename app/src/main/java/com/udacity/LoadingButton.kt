...
class LoadingButton @JvmOverloads constructor(
        ...
        addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationRepeat(animation: Animator?) {
                if (buttonState == ButtonState.Completed) {
                    downloadText = context.getString(R.string.button_download)
                    end()
                }
            }
        })
    ...
    private var buttonState: ButtonState by Delegates.observable<ButtonState>(ButtonState.Completed) { property, old, new ->
        ...
        when (new) {
            ...
            ButtonState.Loading -> {
                // Loading text
                downloadText = context.getString(R.string.button_loading)

                // Progress animator
                valueAnimator = ValueAnimator.ofFloat(0f, measuredWidth.toFloat()).setDuration(2000).apply {
                ...
                // Circle animator
                circleAnimator = ValueAnimator.ofFloat(0f, 360f).setDuration(2000).apply {
                    addUpdateListener { valueAnimator ->
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.drawText(downloadText, textCenterX, textCenterY, textPaint)
